...
def launch_model_final_training(loaded_train_val_data, learning_rate, patience):
    init_patience = patience
    ...
    max_epochs = 1000
    train_val_loss_list = []
    detailed_train_val_loss_list = []
    train_val_loss_prev_epoch = 100
    ...
    for epoch in range(max_epochs):
        ...
        for step, batch in enumerate(loaded_train_val_data):
            input_ids_batch, input_mask_batch, labels_batch = batch
            optimizer.zero_grad()
            # Forward pass:
            train_output = model(input_ids=input_ids_batch, token_type_ids=None,
            ...
            training_loss.append(loss_tensor.item())
            # Also save loss per step for plotting:
            detailed_train_val_loss_list.append(loss_tensor.item())

        train_val_loss_curr_epoch = sum(training_loss) / len(training_loss)

        # Early stopping:
        if epoch > 0 and train_val_loss_curr_epoch > train_val_loss_prev_epoch:
            # Patience is the number of times val loss can be larger than in previous run
            if patience == 0:
                print(f"***Early stopping at: {epoch}/{max_epochs}***")
                break  # Stop early
            patience -= 1
        train_val_loss_prev_epoch = train_val_loss_curr_epoch

        train_val_loss_list.append(train_val_loss_curr_epoch)
        print(f'Training loss: {train_val_loss_curr_epoch}')

    # Save the final trained model config:
    torch.save(model, f'models/final_model_lr{learning_rate}_ptn{init_patience}.pt')  # Save whole model

    return detailed_train_val_loss_list, train_val_loss_list
